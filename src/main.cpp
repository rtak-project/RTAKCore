...
#if defined(NDEBUG)
#error "RTAK cannot be compiled without assertions."
#endif
...
{
    //Check to see if the RTAK is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 RTAK spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend RTAK without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : RTAK spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : RTAK spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : RTAK spend tx %s already in block %d",
            ...
            // Check that RTAK mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight > 0 && nHeight <= 110) {
      nSubsidy = 10 * CENT;
  	} else if (nHeight >= 111 && nHeight <= 20000) {
      nSubsidy = 200 * CENT;
 ...
 */
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isRTAKStake)
{
    int64_t ret = 0;

    if (nHeight > 0 && nHeight <= 110) {
        ret = 40 * CENT;
	  } else if (nHeight >= 111 && nHeight <= 20000) {
        ret = 880 * CENT;
    } else if (nHeight >= 20001 && nHeight <= 40000) {
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from RTAK
...
{
    RenameThread("rtak-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateRTAKMinted()
{
...
void RecalculateRTAKSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from RTAK supply
...
bool RecalculateRTAKSupply(int nHeightStart)
{
...
bool UpdateRTAKSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateRTAKSupply(Params().Zerocoin_StartHeight());
    }

    //Track RTAK money supply in the block index
    if (!UpdateRTAKSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new RTAK supply for block=%s height=%d", __func__,
...
//              FormatMoney(nFees), FormatMoney(pindex->nMint), FormatMoney(nAmountZerocoinSpent));
    ...
    CAmount nExpectedMint = GetBlockValue(pindex->pprev->nHeight + 1) + GetMasternodePayment(pindex->pprev->nHeight + 1, 0, 0, true) + GetDevFee(pindex->pprev->nHeight + 1);
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert RTAK to RTAK
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent RTAK spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of RTAK serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: RTAK stake block is less than allowed start height", __func__);

    if (CZMdsStake* RTAK = dynamic_cast<CZMdsStake*>(stake)) {
        CBlockIndex* pindexFrom = RTAK->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with RTAK stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: RTAK stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(RTAK->GetValue()));
        if (nChecksum200 != RTAK->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, RTAK->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsRTAK() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked RTAK fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d RTAK mints and %d RTAK spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // RTAK: We use certain sporks during IBD, so check to see if they are
