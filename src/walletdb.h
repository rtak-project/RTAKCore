...
public:
    ...
    bool WriteRTAKSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadRTAKSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadRTAKSeed_deprecated(uint256& seed);
    bool EraseRTAKSeed();
    bool EraseRTAKSeed_deprecated();

    bool WriteRTAKCount(const uint32_t& nCount);
    bool ReadRTAKCount(uint32_t& nCount);
