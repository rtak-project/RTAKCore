...
CRTAKWallet::CRTAKWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadRTAKSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseRTAKSeed_deprecated()) {
                    LogPrintf("%s: Updated RTAK seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CRTAKWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteRTAKCount(nCountLastUsed);
    else if (!walletdb.ReadRTAKCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CRTAKWallet::Lock()
{
    seedMaster = 0;
}

void CRTAKWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CRTAKWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToRTAK(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CRTAKWallet::LoadMintPoolFromDB()
{
...
void CRTAKWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CRTAKWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CRTAKWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CRTAKWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToRTAK(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CRTAKWallet::SeedToRTAK(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CRTAKWallet::GetZerocoinSeed(uint32_t n)
{
...
void CRTAKWallet::GenerateDeterministicRTAK(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CRTAKWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CRTAKWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
