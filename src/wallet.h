...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT1000IFMN = 3,
    ONLY_NONDENOMINATED_NOT1000IFMN = 4, // ONLY_NONDENOMINATED and not 1000 RTAK at the same time
    ONLY_1000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for RTAK send
enum ZerocoinSpendStatus {
    RTAK_SPEND_OKAY = 0,                            // No error
    RTAK_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    RTAK_WALLET_LOCKED = 2,                         // Wallet was locked
    RTAK_COMMIT_FAILED = 3,                         // Commit failed, reset status
    RTAK_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    RTAK_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    RTAK_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    RTAK_TRX_CREATE = 7,                            // Everything related to create the transaction
    RTAK_TRX_CHANGE = 8,                            // Everything related to transaction change
    RTAK_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    RTAK_INVALID_COIN = 10,                         // Selected mint coin is not valid
    RTAK_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    RTAK_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    RTAK_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    RTAK_SPENT_USED_RTAK = 14,                      // Coin has already been spend
    RTAK_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    RTAK_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateRTAKOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CRTAKWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CRTAKTracker> mdsTracker;
    ...
    void setZWallet(CRTAKWallet* zwallet)
    {
        zwalletMain = zwallet;
        mdsTracker = std::unique_ptr<CRTAKTracker>(new CRTAKTracker(strWalletFile));
    }

    CRTAKWallet* getZWallet() { return zwalletMain; }
    ...
    /** RTAK reset */
    boost::signals2::signal<void()> NotifyRTAKReset;
