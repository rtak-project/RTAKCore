...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::RTAK);
    ...
    if (!settings.contains("nAnonymizeRTAKAmount"))
        settings.setValue("nAnonymizeRTAKAmount", 1000);

    nAnonymizeRTAKAmount = settings.value("nAnonymizeRTAKAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeRTAKAmount"))
        SoftSetArg("-anonymizertakamount", settings.value("nAnonymizeRTAKAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in rtak.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeRTAKAmount:
            return QVariant(nAnonymizeRTAKAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeRTAKAmount:
            nAnonymizeRTAKAmount = value.toInt();
            settings.setValue("nAnonymizeRTAKAmount", nAnonymizeRTAKAmount);
            emit anonymizeRTAKAmountChanged(nAnonymizeRTAKAmount);
            break;
        case CoinControlFeatures:
