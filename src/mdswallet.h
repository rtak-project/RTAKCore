...
#ifndef RTAK_RTAKWALLET_H
#define RTAK_RTAKWALLET_H
...
public:
    CRTAKWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicRTAK(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToRTAK(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //RTAK_RTAKWALLET_H
