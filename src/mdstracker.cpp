...
CRTAKTracker::CRTAKTracker(std::string strWalletFile)
{
...
CRTAKTracker::~CRTAKTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CRTAKTracker::Init()
{
...
bool CRTAKTracker::Archive(CMintMeta& meta)
{
...
bool CRTAKTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CRTAKTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CRTAKTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CRTAKTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CRTAKTracker::GetSerialHashes()
{
...
CAmount CRTAKTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CRTAKTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CRTAKTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CRTAKTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CRTAKTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CRTAKTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CRTAKTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CRTAKTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CRTAKTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CRTAKTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CRTAKTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CRTAKTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CRTAKTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CRTAKTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CRTAKTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CRTAKTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CRTAKTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
